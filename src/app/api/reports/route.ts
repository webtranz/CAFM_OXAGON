...
async function housingReportRows(type: string, filters: ReturnType<typeof reportFilters>): Promise<ReportRow[]> {
  ...
  if (["housing-bookings", "housing-occupancy-daily", "housing-occupancy-weekly", "housing-occupancy-monthly", "housing-company-occupancy", "housing-building-occupancy", "housing-bed-occupancy", "housing-fm-occupancy"].includes(type)) {
    ...
    if (type === "housing-fm-occupancy") {
      const fmRows = mapped.filter((row) => /\bfm\b|facility management|facilities management|tamimi|housing/i.test(`${row.companyName} ${row.company} ${row.department} ${row.requestedBy}`));
      return applyHousingFilters(fmRows, filters);
    }
